(function(){
  ...
  function mount({snapshot,data,model}){
    ...
    function disable(){try{localStorage.setItem(KEY,'off');enabled=false;clearTimeout(timer);timer=null;status('예약 알림을 껐습니다. 접속 시간과 수집 기록은 보관됩니다.');buttons();}catch{status('알림 설정을 저장하지 못했습니다. 이 페이지를 닫으면 알림이 멈춥니다.');}}
