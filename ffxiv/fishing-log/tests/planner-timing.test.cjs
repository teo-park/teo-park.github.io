...
const MINUTE=60000;
test('collection badges use actual opening time with exact 15/30-minute boundaries and no expired or pending badges',()=>{
 const p=open({plan:true});try{
  ...
  assert.equal(state(start-1),'15');assert.equal(state(start),'now');assert.equal(state(row.end-1),'now');assert.equal(state(row.end),null);
...
test('open collection list updates 30 -> 15 -> now -> expired while retaining conditions and notification preferences',async()=>{
 const p=open({plan:true});try{
