...
const D=JSON.parse(JSON.stringify(context.window.FISHING_DATA)),KEY='teo-ffxiv.fishing.collection.v1';
...
function open({storage=memory(),scan=false,analyze}={}){
 ...
 for(const name of ['data.js','engine.js'])w.eval(fs.readFileSync(path.join(root,name),'utf8'));
 ...
 return {w,d,$,all,change,storage,downloads,scanApply,close:()=>w.close(),async addImage(name='sample.png'){const el=$('#scanFiles');Object.defineProperty(el,'files',{configurable:true,value:[{name,type:'image/png',size:100}]});el.dispatchEvent(new w.Event('change'));await new Promise(r=>setTimeout(r,0));}};
}
