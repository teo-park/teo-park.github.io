...
function open({storage=memory(),scan=false,analyze,plan=false}={}){
 ...
 let planSnapshot;if(plan){w.FishingNotifications={mount:({snapshot})=>{planSnapshot=snapshot;}};for(const name of ['weather-data.js','forecast.js','map-data.js','map-preview.js','planner.js'])w.eval(fs.readFileSync(path.join(root,name),'utf8'));}
