  ...
  function mount({data,model}){
    ...
    function row(item){
      ...
      return `<div class="plan-entry catalog-entry" data-catalog-entry="${id}"><article class="plan-card catalog-card" aria-label="${esc(fish.name)} 수집도감" data-plan-now="false" data-plan-availability="unknown">
        <div class="plan-fish"><img src="${esc(fish.icon)}" width="30" height="30" alt="" loading="lazy"><div><button class="plan-name" data-fish-detail="${id}">${esc(fish.name)}</button><div class="plan-labels"><span>No.${fish.order}</span><span>${window.FishingBook.isOceanLegendary(fish)?'먼바다 전설어':window.FishingBook.isLegendary(fish)?'터주왕':fish.big?'터주':fish.kind==='spear'?'작살질':'일반'}</span><span class="catalog-collected">수집 완료</span><span data-catalog-timing></span></div></div></div>
