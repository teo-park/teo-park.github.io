(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.FishingBook=api;})(typeof globalThis==='object'?globalThis:this,function(){
  ...
  // Field legendary fish (터주왕) and ocean five-star fish are separate groups.
  const isOceanLegendary=fish=>fish.kind==='rod'&&fish.stars===5;
  const isLegendary=fish=>!!fish.legendary&&!isOceanLegendary(fish);
  ...
  function create(data,biteTimes={}){
    ...
    const rarityGroup=fish=>isOceanFish(fish)?(isOceanLegendary(fish)?'oceanLegendary':fish.big?'oceanBig':'normal'):(isLegendary(fish)?'legendary':fish.big?'fieldBig':'normal');
    ...
    function filter(caught,options={}){const q=normalize(options.query),number=/^(?:no)?\d+$/.test(q)?+q.replace(/^no/,''):null;
      return data.fishes.filter(f=>scopeMatches(f,options.scope)&&(!options.kind||f.kind===options.kind)&&(!options.status||options.status==='all'||caught.has(f.id)===(options.status==='caught'))&&(!options.rarity||options.rarity==='all'||(options.rarity==='normal'?!f.big:options.rarity==='big'?f.big:rarityGroup(f)===options.rarity))&&(!q||(number!==null?f.order===number||f.id===number:terms.get(f.id).text.includes(q)||terms.get(f.id).initials.includes(q)))&&((options.region==='all'||!options.region)&&((options.bait==='all'||!options.bait)||options.bait==='unknown'&&!f.routes.length)||routeList(f,options).length));
    }
    ...
    return {byId,isOceanFish,rarityGroup,paths,tacklePaths,biteTime,baitOptions,competitors,routeMatches,routeList,filter,groups};
  }
  ...
  return {normalize,initials,isLegendary,isOceanLegendary,comparisonTugs,lureRequirements,create,parseBackup,parseTransfer,backup,teamcraft,timeWindow,clearBiteWindows,biteStats};
});
