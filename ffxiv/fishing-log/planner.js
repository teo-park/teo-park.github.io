(function(){
  ...
  function mount({data,model,getCaught}){
    ...
    function card(row){
      ...
      return `<article class="plan-card" data-plan-kind="${fish.big?'big':'normal'}" data-plan-availability="${row.always?'always':'timed'}"><div class="plan-fish"><img src="${esc(fish.icon)}" width="40" height="40" alt="" loading="lazy"><div><button class="plan-name" data-fish-detail="${fish.id}">${esc(fish.name)}</button><p>${esc(spot.area)} · ${esc(spot.name)}</p></div><button class="plan-star" data-plan-star="${fish.id}" aria-pressed="${stars.has(fish.id)}" aria-label="${esc(fish.name)} 관심 물고기">${stars.has(fish.id)?'★':'☆'}</button></div><div class="plan-labels"><span>${fish.big?(fish.legendary?'전설어':'터주'):'일반 물고기'}</span><span>${row.always?'상시':'시간·날씨 조건'}</span></div><div class="plan-window">${row.always?'<strong>상시 낚시</strong><span>시간·날씨 제한 없음 · 원하는 접속 시간에 도전</span>':`<strong>${date.format(row.start)} – ${time.format(row.end)}</strong><span>${Math.floor((row.end-row.start)/F.MINUTE)}분 도전${row.start<=result.now?' · 현재 시간부터':''}</span>`}</div><p class="plan-bait">${esc(chain(route))}</p>${special?`<p class="plan-condition">직감 준비: ${esc(special)} · <button data-fish-detail="${fish.id}">조건 확인</button></p>`:''}${row.always?'':`<p class="plan-next">이 기회를 놓치면 · 내 다음 접속 기회 ${next}</p>`}<div class="plan-card-actions"><button data-fish-detail="${fish.id}">낚시 조건</button><button data-caught="${fish.id}">수집 체크</button></div></article>`;
    }
    function prep(list){
