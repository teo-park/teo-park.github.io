(function(){
  ...
  function mount({data,model,getCaught}){
    ...
    function card(row){
      ...
      const coords=Number.isFinite(spot.coords?.x)&&Number.isFinite(spot.coords?.y)?`<span class="plan-spot-coords" title="Teamcraft 낚시터 좌표">X:${spot.coords.x.toFixed(1)} · Y:${spot.coords.y.toFixed(1)}</span>`:'';
