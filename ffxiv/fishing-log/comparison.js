(function(){
  ...
  function mount({model,reference}){
    function render(route,fish,index,excludedId=0,compact=false){
      ...
      const allowed=window.FishingBook.comparisonTugs(route),expanded=allowed.length>1,title=expanded?'낚아채기 비교':'같은 입질 비교',scope=expanded?hooks[route.hookset]+' · '+allowed.map(t=>tugs[t]).join(' / '):tugs[route.tug];
      const known=c=>c.baitKnown&&c.tugKnown&&c.hooksetKnown!==false;
      const selectable=competitors.filter(known);
      if(mooch||!selectable.some(c=>c.fish.id===excludedId))excludedId=0;
      const remaining=competitors.filter(c=>c.fish.id!==excludedId),uncertain=remaining.filter(c=>!known(c)||!c.time);
      const windows=window.FishingBook.clearBiteWindows(target,remaining),knownCount=remaining.filter(known).length;
      ...
      const row=(entry,isTarget=false)=>{
        ...
        const state=isTarget?'대상':excluded?'제외 가정':!entry.baitKnown?'미끼 확인 필요':!entry.tugKnown?'입질 확인 필요':entry.hooksetKnown===false?'낚아채기 확인 필요':!time||!target?'시간 미확인':overlap?'시간 겹침':'관측 시간 분리';
      ...
      const confirmed=competitors.filter(known),unconfirmed=competitors.filter(c=>!known(c));
      if(compact){
        ...
        return `<section class="bite-comparison compare-compact" data-compare-compact="true" data-compare-fish="${fish.id}" data-compare-route="${index}" aria-label="${esc(fish.name)} ${title}">
          <div class="compare-compact-heading"><h4>${title} <span>${entries.length}종</span></h4><span>${esc(model.byId.get(route.bait)?.name||route.bait)} · ${scope} · 대상 ${rangeText(target)}</span></div>
          ...
          ${entries.length?table(entries.slice(0,4)):'<p class="compare-rule">비교할 다른 어종 기록 없음</p>'}
      ...
      return `<section class="bite-comparison" data-compare-fish="${fish.id}" data-compare-route="${index}" aria-label="${esc(fish.name)} ${title}">
        <h4>${title}</h4><p class="compare-context">${esc(model.byId.get(route.bait)?.name||route.bait)}${mooch?' 생미끼':''} · ${scope} · 같은 낚시터 기준</p>
