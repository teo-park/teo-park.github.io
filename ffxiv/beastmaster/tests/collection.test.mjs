...
test('details distinguish starter quest from capture and merchant coordinates from unverified capture locations',()=>{
  const a=setup();try{
    ...
    for(const expected of ['크리스탈 타워: 고대인의 미궁','고순도 마수핵 40개','탐식의 거틀러','상인 위치','X:21.9 · Y:22.6','세부 포획 좌표는 아직 확인되지'])assert.ok(text.includes(expected),expected);
...
test('missing image leaves a readable numbered fallback',()=>{
  const a=setup();try{
    ...
    assert.match(a.$('appContent').textContent,/신규 이미지가 없으면 번호/);
  }finally{a.close();}
});
