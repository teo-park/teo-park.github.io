import {elements,purposes,ailments,commands,combatActions,purposeMatches,weaknessMatches} from './combat.js?v=20260909-borrow1';
...
export function initCombatFilters(doc,onChange){
  doc.getElementById('combatCommand').innerHTML='<option value="all">모든 기술</option>'+Object.entries(commands).map(([key,c])=>`<option value="${key}">${c.name}만</option>`).join('');
  ...
  for(const id of ['combatCommand','combatPurpose','combatWeakness'])doc.getElementById(id).onchange=onChange;
}
...
function skill(action,o={},full=false){
  ...
  return `<div class="combat-skill ${matched?'skill-matched':''}" data-command="${action.kind}"><div class="combat-skill-title"><span>${commands[action.kind].name} · Lv.${action.level}${action.family?` · ${esc(action.family)}`:''}</span><strong>${esc(action.name)}</strong>${matched?'<b class="match-label">조건 일치</b>':''}</div><div class="combat-badges">${badges(action,o)}</div><p class="combat-effect">${esc(full?action.summary:shortEffect(action))}</p></div>`;
}
...
export function combatRow(beast,profile,o){
  ...
  return `<article class="combat-row ${actions.length===1?'single-command':''}" data-combat-beast="${beast.id}"><div class="combat-beast"><span class="combat-number">No.${beast.id}</span><h3>${esc(beast.name)}</h3><button class="combat-check" data-check="${beast.id}" aria-pressed="false"><span class="check-mark" aria-hidden="true">＋</span> <span class="state-label">미수집</span></button><button class="quiet" data-detail="${beast.id}">기술·획득처 ↗</button></div>${actions.map(a=>skill(a,o)).join('')}</article>`;
}
export function combatDetails(beast,profile,data){
  ...
  return `<section class="combat-detail"><h2 class="section-title">용도·전투 기술</h2><p class="muted combat-scope">궁극기·능력 해방과 박물학적 분류에 따른 빌리기 효과입니다. 적에게 표시된 약점과 공격 속성을 맞춰 보세요.</p>${profile.actions.map(a=>skill(a,{},true)).join('')}<p class="combat-scope">궁극기는 사역마수 TP 100 이상, 능력 해방은 사역마수가 생존한 인간 마수 동조 중에 전투 중 사용합니다. 상태 이상은 대상의 저항을 확인하세요. Lv.44부터 능력 해방의 마지막 일격 효과 향상도 적용됩니다.</p><p class="combat-scope">빌리기(Lv.22)는 사역마수 생존 및 인간 마수 동조 중 사용합니다. 빌리기로 본능을 얻은 뒤, 변화한 ‘마수 기술’을 사용하세요. 사역마수가 귀환하면 본능의 남은 시간은 90초가 됩니다. Lv.46부터 빌리기에도 마지막 일격 효과 향상이 적용됩니다.</p><p class="combat-source">${link(guide,'공식 전투 안내')} · ${link(source,'기술 효과 원문')} · Action ${profile.actions.map(a=>a.id).join(' / ')}</p></section>`;
}
