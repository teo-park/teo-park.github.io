...
(function (root, factory) {
  ...
  const refCentered = REFERENCE.map(n => n - refMean);
  const refNorm = Math.sqrt(refCentered.reduce((sum, n) => sum + n * n, 0));
  ...
  function descriptor(image, cx, cy, width, height, angle = 0) {
    const values = [];
  ...
  function compare(values) {
    ...
    for (let i = 0; i < values.length; i++) { const n = values[i] - average; variance += n * n; dot += n * refCentered[i]; }
    const norm = Math.sqrt(variance);
    return {score: norm > 0 ? dot / (norm * refNorm) : 0, texture: norm / SIZE};
  }
  function classifyCell(image, cx, cy, width, height) {
    let best = {score: -1, texture: 0}, fit;
    for (const scale of [.66, .74, .82]) for (const dx of [-.055, 0, .055]) for (const dy of [-.055, 0, .055]) {
      const result = compare(descriptor(image, cx + dx * width, cy + dy * height, width * scale, height * scale));
      if (result.score > best.score) { best = result; fit = {cx: cx + dx * width, cy: cy + dy * height, scale}; }
    }
    if (best.score >= .5 && best.score < .97) {
      for (const angle of [-12, -6, 0, 6, 12]) for (const dx of [-.035, 0, .035]) for (const dy of [-.035, 0, .035]) {
        const candidate = compare(descriptor(image, fit.cx + dx * width, fit.cy + dy * height, width * fit.scale, height * fit.scale, angle));
  ...
  function analyze(image, rect, count = 30) {
    ...
    return Array.from({length: count}, (_, index) => ({index, ...classifyCell(image, rect.x * image.width + (index % 5 + .5) * w, rect.y * image.height + (Math.floor(index / 5) + .5) * h, w, h)}));
  }
  ...
  return {SIZE, descriptor, compare, classifyCell, validateRect, analyze, pagesFor, prepareImport};
});
