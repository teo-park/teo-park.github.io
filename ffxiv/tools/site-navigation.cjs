...
const guidePages = [['fisher-skills/big-fish/', '터주 유형별 공략']];
const pages = ['', ...categories.flatMap(c => c.tools.map(([url]) => url)), ...journalPages.map(([url]) => url), ...guidePages.map(([url]) => url), 'ocean-fishing/sources/'];
...
function header(page) {
  ...
  const groups = categories.map(category => {
    ...
    return `<details class="nav-category"${active ? ' data-current-category' : ''}><summary aria-controls="nav-${category.id}">${category.label}<svg viewBox="0 0 16 16" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6"><path d="m4 6 4 4 4-4"/></svg></summary><div class="nav-dropdown" id="nav-${category.id}"><ul>${category.tools.map(tool => link(tool) + (tool[0] === 'ocean-fishing/' ? `<li class="nav-journal-pages"><ul aria-label="항해일지 페이지">${journalPages.map(tool => link(tool, true)).join('')}</ul></li>` : tool[0] === 'fisher-skills/' ? `<li class="nav-journal-pages"><ul aria-label="어부 가이드 페이지">${guidePages.map(tool => link(tool, true)).join('')}</ul></li>` : '')).join('')}</ul></div></details>`;
  }).join('');
