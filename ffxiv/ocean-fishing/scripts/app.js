...
(() => {
  ...
  function planned() {
    ...
    return result.filter(f=>keep.has(C.key(f.Fish))).map(f=>{
      const requiredBy=[...new Set([...(f.LocalRequiredBy||[]),...(required.get(C.key(f.Fish))||[])])];
      // Compare prerequisites against the full voyage, before filters hide competing catches.
      const compare=requiredBy.length>0||(purpose==='achievement'&&f.LocalGroupMatch);
      return {...f,LocalRequiredBy:requiredBy,LocalBiteComparison:compare?A.biteComparison(f,available):null};
    });
  }
