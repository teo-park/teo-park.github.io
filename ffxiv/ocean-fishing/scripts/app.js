...
(() => {
  ...
  let purpose = 'collection', species = [], detail = read('ocean:table-details') === 'true';
  ...
  function table(rows, id, options={}) {
    const score=purpose==='score' && !isChecklist, extra=detail || isChecklist;
    ...
    const headers=['물고기','입질','권장 미끼',...(score||extra?['기본 점수','이중 점수','삼중 점수']:[]),...(extra?['시간·날씨','물고기군','미끼별 입질 시간']:[])];
  ...
  function bind() {
    ...
    if(isChecklist) {
      ...
      $('showDetails').addEventListener('change',event=>{detail=event.target.checked;write('ocean:table-details',String(detail));if(!detail)for(const [id,options]of zoneOptions)zoneOptions.set(id,{...options,bait:''});renderFishing();});
