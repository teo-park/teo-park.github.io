...
// Resolve display names from stable stop codes, shared with route translations.
function getFishingStopLabel(stop, fallback) {
	var keys = {
		galadion: 'destination.galadionbay',
		southern: 'destination.thesouthernstraitofmerlthor',
		rhotano: 'destination.rhotanosea',
		northern: 'destination.thenorthernstraitofmerlthor',
		cieldalaes: 'destination.thecieldalaes',
		bloodbrine: 'destination.thebloodbrinesea',
		blood: 'destination.thebloodbrinesea',
		rothlyt: 'destination.therothlytsound',
		sirensong: 'destination.thesirensongsea',
		kugane: 'destination.kugane',
		'ruby sea': 'destination.therubysea',
		'one river': 'destination.theoneriver',
		unnamed: 'destination.unnamed',
		thavnair: 'destination.thavnair'
	};
	var key = keys[String(stop || '').trim().toLowerCase()];
	var label = key ? translateWord(key) : '';
	return label && label !== key ? label : String(fallback || stop || '').trim();
}
