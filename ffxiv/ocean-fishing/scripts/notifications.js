(() => {
  ...
  const LOCK='teo-ffxiv-ocean-departure-alert',GRACE=90000;
  ...
  function mount(){
    const button=document.getElementById('oceanNotifications'),hint=document.getElementById('oceanNotificationStatus');
    ...
    function update(text){
      ...
      button.textContent=busy?'알림 설정 중…':'정시 알림 '+(enabled?'ON':'OFF');hint.textContent=message;
    ...
    const current=now=>V.upcoming('indigo',now,1)[0].start;
    ...
    function show(start){
      const routes=[['indigo','근해'],['ruby','원양']].map(([route,label])=>{
    ...
    async function check(){
      ...
      }catch{stop('알림을 표시하거나 이력을 저장하지 못해 정시 알림을 껐어요. 브라우저 권한·저장 공간을 확인해 주세요.',true);}
    ...
    function disable(){++revision;stop('정시 알림 OFF · 근해·원양 공통',true);}
    function sync(){
      ++revision;
      try{
        ...
        else stop(Notification.permission==='denied'?'알림이 차단되어 있어요. 브라우저 사이트 설정에서 허용해 주세요.':'정시 알림 OFF · 근해·원양 공통');
