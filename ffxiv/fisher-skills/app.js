(function(){
  ...
  function revealHash(){const id=decodeURIComponent(location.hash.slice(1)),target=document.getElementById(id);if(target?.hidden||target?.closest('[data-skill-group]')?.hidden){category='all';search.value='';render();}if(target)target.scrollIntoView({block:'start'});}
