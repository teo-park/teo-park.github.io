(function(){
  'use strict';
  const $=s=>document.querySelector(s),cards=[...document.querySelectorAll('.skill-card')],search=$('#skillSearch');
  ...
  function render(){const q=normalize(search.value);let count=0;for(const card of cards){const t=texts.get(card),show=(category==='all'||card.dataset.category===category)&&(!q||t.text.includes(q)||t.initials.includes(q));card.hidden=!show;if(show)count++;}$('#skillCount').textContent=`${count}개 설명 · 전체 ${cards.length}개`;$('#noSkills').hidden=count>0;for(const b of document.querySelectorAll('[data-filter]'))b.setAttribute('aria-pressed',String(b.dataset.filter===category));}
