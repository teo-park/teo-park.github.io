...
- 각 칸을 전체 583종 아이콘의 색상·형태 특징과 비교합니다. 미수집 대상이 생략되어도 위치나 페이지 번호로 꼬친 ID를 추정하지 않습니다.
- 기본으로 **게임 분류순으로 후보 보정**을 적용합니다. 확실하게 인식한 앞뒤 꼬친 사이의 전체 목록에서 그림을 다시 비교하므로 최초 8개 후보에 빠진 대상도 복구합니다. 순서만으로 확정하지 않으며 그림이 충분히 맞지 않거나 순서가 충돌하면 확인 필요로 남깁니다. 보정된 칸에는 근거가 된 앞뒤 이름을 표시합니다. 즐겨찾기·검색 등 다른 정렬의 캡처에서는 이 옵션을 끌 수 있습니다.
