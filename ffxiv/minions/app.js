...
(function(){
  ...
  function tile(m){const checked=owned.has(m.id),source=m.sources[0];return `<article class="minion-tile ${checked?'is-owned':''}" data-id="${m.id}"><button type="button" class="collect-button" data-collect="${m.id}" aria-label="${esc(m.name)} 보유" aria-pressed="${checked}"><span class="collection-tick" aria-hidden="true">✓</span><img src="${esc(m.icon)}" alt="" width="52" height="52" loading="lazy"><span class="minion-name">${esc(m.name)}</span><span class="collection-state" aria-hidden="true">${checked?'보유':'미수집'}</span></button><div class="collection-row-info"><span>${source?esc(source.typeName+' · '+source.name)+(m.sources.length>1?' 외 '+(m.sources.length-1)+'곳':''):'획득처 확인 중'}</span><small>패치 ${esc(m.patch)} · ${m.tradeable?'거래 가능':'거래 불가'}</small></div><button type="button" class="source-button" data-detail="${m.id}" aria-label="${esc(m.name)} 획득처">획득처 ↗</button></article>`;}
  function renderList(){
